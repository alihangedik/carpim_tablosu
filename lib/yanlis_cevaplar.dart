...
class YanlisSorularProvider extends ChangeNotifier {
  ...
  Future<void> addYanlisSoru(Map<String, dynamic> soru) async {
    ...
    await _saveYanlisSorularToPrefs(); // Önce kaydet
    notifyListeners(); // Sonra state güncelle
  }
  ...
  Future<void> _saveYanlisSorularToPrefs() async {
    ...
    String jsonString =
        json.encode(_yanlisSorular); // Listeyi JSON formatında dönüştür
    await prefs.setString('yanlisSorular',
        jsonString); // Veriyi shared_preferences'a kaydediyoruz
  }
  ...
  Future<void> loadYanlisSorularFromPrefs() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? jsonString =
        prefs.getString('yanlisSorular'); // Veriyi String olarak alıyoruz
...
class _YanlisSorularTabViewState extends State<YanlisSorularTabView> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    Provider.of<YanlisSorularProvider>(context, listen: false)._saveYanlisSorularToPrefs();
  }

  // Tek bir fonksiyonla Dismissible widget'ı tekrar kullanabilmek
  Widget _buildSoruListesi(String islemTuru) {
    return Consumer<YanlisSorularProvider>(
      builder: (context, provider, child) {
        var sorular = provider.filtreleByIslemTuru(islemTuru);
        return sorular.length == 0 ? Center(child: Column(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Dismissible(
                    ...
                    onDismissed: (direction) {
                      ...
                      provider._saveYanlisSorularToPrefs();
                      setState(() {});
                    },
